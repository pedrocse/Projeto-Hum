        ...
        if opcao == "Visão Geral":
            ...
            if st.button("Buscar Estudante"):
                ...
                if not aluno_df.empty:
                    ...
                    st.plotly_chart(fig)
                    # Novo gráfico hexagonal
                    st.subheader(f"Diagrama Hexagonal de Habilidades - {bimestre}")
                    fig_hex = go.Figure()

                    # Preparar dados
                    disciplinas_hex = list(disciplinas) + [disciplinas[0]]  # Fechar o hexágono
                    notas_hex = list(notas) + [notas[0]]
