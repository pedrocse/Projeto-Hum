...
col1, col2 = st.columns([3, 1])  # A coluna esquerda (col1) é maior, e a direita (col2) é menor

# Conteúdo da coluna esquerda (título principal)
with col1:
    st.title("Dashboard de Notas dos Alunos")
    st.write("Este é um dashboard interativo para visualização das notas dos alunos.")

# Conteúdo da coluna direita (logo)
with col2:
    # Carregar e exibir a imagem (logo)
    st.image("humanitas.PNG", width=150)  # Substitua "logo.png" pelo caminho da sua imagem
...
if uploaded_file is not None:
    ...
    bimestre = st.text_input("Digite a turma referente aos dados (ex.: T3):", value="")

    if bimestre.strip() == "":
        st.warning("Por favor, insira a turma antes de continuar.")
    else:
        ...
        with tab2:
            st.header(f"Busca de Aluno - {bimestre}")
            st.write(f"Esta aba permite buscar um aluno específico e visualizar seus gráficos para o {bimestre}.")
            ...
            if st.button("Buscar Aluno"):
                ...
                if not aluno_df.empty:
                    ...
                    st.subheader(f"Distribuição Normal e Notas do Aluno - {bimestre}")
                    ...
                    fig.update_layout(
                        title=f"Distribuição Normal e Notas do Aluno - {bimestre}",
                        xaxis_title="Notas",
        ...
        with tab3:
            st.header(f"Estatísticas da Turma - {bimestre}")
        ...
        with tab4:
            st.header(f"Ranking de Alunos - {bimestre}")
            st.write(f"Esta aba classifica os alunos com base na média geral para o {bimestre}.")
            ...
            # Exibir o ranking
            st.subheader(f"Ranking de Alunos por Média Geral - {bimestre}")
            ...
            fig_ranking.update_layout(
                title=f"Ranking de Alunos por Média Geral - {bimestre}",
                xaxis_title="Alunos",
        ...
        with tab5:
            st.header(f"Comparação de Alunos - {bimestre}")
            st.write(f"Esta aba permite comparar as notas de dois ou mais alunos para o {bimestre}.")
            ...
            if len(alunos_selecionados) >= 2:
                ...
                st.subheader(f"Notas dos Alunos Selecionados - {bimestre}")
                ...
                fig_comparacao.update_layout(
                    title=f"Comparação de Notas dos Alunos - {bimestre}",
        ...
        with tab6:
            st.header(f"Ranking por Disciplina - {bimestre}")
            st.write(f"Esta aba permite selecionar uma disciplina e visualizar as notas de todos os alunos para o {bimestre}.")
            ...
            st.subheader(f"Notas dos Alunos em {disciplina_selecionada} - {bimestre}")
            ...
            fig_barras.update_layout(
                title=f"Notas dos Alunos em {disciplina_selecionada} - {bimestre}",
                xaxis_title="Alunos",
                yaxis_title="Notas",
                height=600
            )
